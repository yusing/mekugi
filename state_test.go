...
func TestMekugi2FinalStateReport(t *testing.T) {
	...
	edits := []FileEdit{{Path: "file.txt", Script: "type " + row(2, "beta") + ` "B"`}}
...
func TestMekugi2FinalStateReportProvidesReusableReplacementTarget(t *testing.T) {
	...
	before := row(2, "beta")
	after := row(2, "B")
	firstEdits := []FileEdit{{Path: "file.txt", Script: "type " + before + ` "B"`}}
	translated, err := TranslateForHostAt(t.Context(), root, firstEdits, "")
	if err != nil {
		t.Fatal(err)
	}
	wantAlias := TargetAlias{Path: root + "/file.txt", Before: before, After: after}
	...
	result, err := applyForHostAtTest(t, root, firstEdits, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}

	rewritten := []FileEdit{{Path: "file.txt", Script: "type " + after + ` "C"`}}
...
func TestMekugi2FormattedReferencesTrackEditedContent(t *testing.T) {
	...
	edits := []FileEdit{{Path: "file.go", Script: "type " + row(7, "var target=1") + ` "var target=2"`}}
	...
	wantAlias := TargetAlias{Path: root + "/file.go", Before: row(7, "var target=1"), After: row(12, "var target = 2")}
	if len(translated.TargetAliases) != 1 || translated.TargetAliases[0] != wantAlias {
...
func TestMekugi2FinalStateReportIsBounded(t *testing.T) {
	...
	edits := []FileEdit{{Path: "file.txt", Script: "type " + row(1, "x x x x") + ` "x" 4 "y"`}}
	result, err := applyForHostAtTest(t, root, edits, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}
...
func TestMekugi2InsertionReportNamesTargetRange(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "file.txt", "alpha\nbeta\n", 0o644)
	edits := []FileEdit{{Path: "file.txt", Script: "add " + row(2, "beta") + ` "inserted\n"`}}
	result, err := applyForHostAtTest(t, root, edits, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}
	...
	if !strings.Contains(result.Report, "refs 1 add\n") {
		t.Fatalf("report = %q", result.Report)
	}
}
...
func TestMekugi2PreviewDoesNotInventTrailingEmptyLine(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "file.txt", "alpha\n", 0o644)
	result, err := applyForHostAtTest(t, root, []FileEdit{{Path: "file.txt", Script: ""}}, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}
...
func TestMekugi2FinalReferencesCoverCommandsFilesAndContinuation(t *testing.T) {
	...
	edits := []FileEdit{
		{Path: "first.txt", Script: "type " + row(2, "a1") + " \"A1\"\ntype " + row(4, "a3") + " \"A3\""},
		{Path: "second.txt", Script: "add " + row(3, "b2") + " \"inserted\\n\""},
	}
	result, err := applyForHostAtTest(t, root, edits, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	...
	continuation, continuationErr := applyForHostAtTest(t, root, []FileEdit{{Path: "second.txt", Script: "type " + reportedTarget + " \"B2\""}}, "")
	if continuationErr != nil {
		t.Fatalf("reported-row continuation error = %v, report %q", continuationErr, continuation.Report)
	}

	stale, staleErr := applyForHostAtTest(t, root, []FileEdit{{Path: "first.txt", Script: "type " + row(2, "a1") + " \"stale\""}}, "")
...
func TestMekugi2FinalReferencesProjectCollapsedDeletion(t *testing.T) {
	root := t.TempDir()
	writeTestFile(t, root, "file.txt", "alpha\nbeta\ngamma\n", 0o644)
	result, err := applyForHostAtTest(t, root, []FileEdit{{Path: "file.txt", Script: "type " + row(2, "beta") + ` ""`}}, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}
	want := "refs 1 type\n" +
...
func TestMekugi2FinalReferencesAreBoundedAndDeduplicated(t *testing.T) {
	...
	edits := []FileEdit{{Path: "file.txt", Script: "type " + row(2, "x") + ` "x" 2 "y"`}}
	result, err := applyForHostAtTest(t, root, edits, "")
	if err != nil {
		t.Fatalf("ApplyForHost() error = %v, report %q", err, result.Report)
	}
...
func TestCompactReportDoesNotRepeatSingleFilePath(t *testing.T) {
	...
	result, err := TranslateForHostAt(t.Context(), root, []FileEdit{{Path: path, Script: `type "package p" "package q"`}}, "")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, want := range []string{"file " + path + "\n", "last type 1 ranges ", "refs 1 type\n", "format (pre-format -> final)\n", "Formatted "} {
		if !strings.Contains(result.Report, want) {
