...
func TestCodeModeSpreadBatchActivityProjectsCommandsJSONAndSSE(t *testing.T) {
	...
	for _, projection := range []struct{ name, source string }{
		{"forEach", `r.forEach((v,i)=>text(JSON.stringify({i,...v})));`},
		{"indexed", `for(let i=0;i<r.length;i++)text(JSON.stringify({i,...r[i]}));`},
	} {
		source := batch + projection.source
		for _, stream := range []bool{false, true} {
			t.Run(projection.name+"/"+map[bool]string{false: "json", true: "sse"}[stream], func(t *testing.T) {
				p := newManagedMekugiProxy(t)
				root, _ := prepareActivityTest(t, p, "root-session", "root-thread", "", "/root", nil)
				...
				for _, leaked := range []string{"const r=await Promise.allSettled", "r.forEach", "for(let", "JSON.stringify", "tools.exec_command"} {
...
func TestSiblingReceiptAndOpaqueCallsKeepExactEnvelope(t *testing.T) {
	...
	excerpt := body[:511] + "…"
	if bytes.Contains(output, []byte("Follow-up requested.")) || !bytes.Contains(output, []byte("opaque-secret")) || !bytes.Contains(output, []byte(excerpt)) || bytes.Contains(output, []byte(body)) {
