...
`, pluginID, toolName, formatField)
	...
	t.Run("missing directory loads embedded built-ins", func(t *testing.T) {
		...
		for _, name := range []string{"mchanges", "mread", "mrun", "mcat", "hgrep", "msymbol", "inspect_file", "shell"} {
			_, ok := registry.contribution(name)
			if !ok {
				t.Fatalf("built-in %q is unavailable", name)
			}
		}
		for _, name := range []string{mekugiToolName, "shell"} {
		...
		for _, name := range []string{"mchanges", "mread", "mrun", "mcat", "hgrep", "msymbol", "inspect_file"} {
