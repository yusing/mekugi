...
func TestReplayContinuationMatchesLiveProviderInput(t *testing.T) {
	...
	for _, carrier := range []string{"native", "code-mode"} {
		for _, tool := range []string{"hpatch", "shell"} {
			t.Run(carrier+"/"+tool, func(t *testing.T) {
				...
				first := request(nil)
				if err := executeRequest(t.Context(), t.Context(), first, headers, "parent", provider, &output, nil, proxy, nil); err != nil {
					t.Fatal(err)
				}
				...
				for _, continuation := range []string{"live", "fork", "resume", "resumed-fork"} {
					...
					output.Reset()
					if err := executeRequest(t.Context(), t.Context(), request(history), headers, continuation, provider, &output, nil, proxy, nil); err != nil {
