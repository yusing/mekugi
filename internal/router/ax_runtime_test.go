...
func TestAXObservesExecutedReaders(t *testing.T) {
	t.Parallel()
	...
	script := "printf 'mcat is only an example\n'; " +
		"if false; then mcat /not-executed; fi; " +
		"mcat " + shellQuoteArgument(path) + "; " +
		"inspect_file " + shellQuoteArgument(path) + "; " +
		"mcat " + shellQuoteArgument(filepath.Join(root, "missing")) + "; true"
	...
	if code != 0 || strings.Count(stdout, "private-source-marker") != 1 ||
		!strings.Contains(stdout, "mcat is only an example") || !strings.Contains(stderr, "ENOENT") {
	...
	if err != nil || reads.Started != 3 || reads.Succeeded != 2 || reads.Failed != 1 ||
		reads.ByTool["mcat"] != 2 || reads.ByTool["inspect_file"] != 1 {
...
func TestAXWriteFailureDoesNotChangeReaderOutcome(t *testing.T) {
	...
	stdout, stderr, code := runShellWorkerTest(t, registry, "bash", nil, "mcat "+shellQuoteArgument(path), nil, invocation)
