...
the same evaluated-input and completed-diff records. Direct shell file creation, moves,
removals, external formatters, and unrelated workspace changes are not tracked. Historical edits to retained `@shell/` scripts are labeled separately from workspace
